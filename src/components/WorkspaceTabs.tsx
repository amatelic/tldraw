...
export function WorkspaceTabs({
  ...
  const [overflowRenameError, setOverflowRenameError] = useState<string | null>(null);
  ...
  function submitOverflowRename(workspaceId: string) {
    const validation = validateWorkspaceName(overflowEditValue);
    if (validation.error) {
      setOverflowRenameError(validation.error);
      return;
    }

    const didRename = onRename(workspaceId, validation.name);
    if (didRename === false) {
      setOverflowRenameError('Workspace name could not be saved.');
      return;
    }
                    ...
                    return (
                      ...
                      >
                        {isEditingOverflowItem ? (
                          <div className="workspace-overflow-rename-editor">
                            <input
                              ...
                              aria-describedby={
                                overflowRenameError
                                  ? `workspace-overflow-rename-error-${workspace.id}`
                                  : undefined
                              }
                              onChange={(event) => {
                                setOverflowEditValue(event.target.value);
                                if (overflowRenameError) {
                                  setOverflowRenameError(null);
                                }
                              }}
                            ...
                            {overflowRenameError && (
                              <div
                                id={`workspace-overflow-rename-error-${workspace.id}`}
                                className="workspace-rename-error workspace-overflow-rename-error"
                                role="alert"
                              >
                                {overflowRenameError}
                              </div>
